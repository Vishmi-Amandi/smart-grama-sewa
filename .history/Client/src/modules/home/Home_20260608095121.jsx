    ...
    <div style={{ backgroundColor: 'var(--bg-page)', color: 'var(--text-main)' }} className="min-h-screen flex flex-col font-sans relative overflow-x-hidden">
      ...
      <nav style={{ backgroundColor: 'var(--bg-topbar)' }} className="flex justify-between items-center py-3 px-4 md:px-8 shadow-sm sticky top-0 z-50">
        ...
        <div className="text-xs md:text-sm flex items-center gap-1 md:gap-2 font-bold ml-auto md:ml-0">
          <button 
            ...
            className={`cursor-pointer px-1 transition-all ${i18n.language === 'si' ? 'text-[#6A2301] font-extrabold scale-105' : 'text-inherit opacity-70 font-semibold'}`}
          >
            සිං
          </button> 
          ...
          <button 
            ...
            style={{ background: 'none', border: 'none' }}
            className={`cursor-pointer px-1 transition-all ${i18n.language === 'ta' ? 'text-[#6A2301] font-extrabold scale-105' : 'text-inherit opacity-70 font-semibold'}`}
          >
            தமிழ்
          </button> 
          <span className="opacity-30 text-xs">|</span>
          <button 
            ...
            style={{ background: 'none', border: 'none' }}
            className={`cursor-pointer px-1 transition-all ${i18n.language === 'en' ? 'text-[#6A2301] font-extrabold scale-105' : 'text-inherit opacity-70 font-semibold'}`}
          >
            EN
          </button>
        </div>
      ...
      >
        <div className="max-w-2xl text-white py-12 md:py-24">
          <h1 className="text-3xl md:text-5xl font-black mb-4 md:mb-6 leading-tight whitespace-pre-line tracking-tight">
            {t('hero_title')}
          </h1>
          <p className="text-sm md:text-lg mb-6 md:mb-8 font-medium text-gray-100 opacity-90 max-w-xl">
            {t('hero_desc')}
          </p>
          <Link to="/gn-login" className="bg-[#FFCB05] text-black px-8 md:px-10 py-3 rounded-full font-extrabold text-sm md:text-base shadow-lg hover:bg-yellow-500 hover:scale-[1.02] active:scale-[0.98] transition-all inline-block text-center">
            {t('btn_get_started')}
          </Link>
        </div>
      </main>
