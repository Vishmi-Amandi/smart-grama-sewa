...
const About = () => {
  // Pass 'translation' as the default namespace array to bypass the nested layer mapping
  const { t, i18n } = useTranslation();
  // TEMPORARY LIVE DEBUGGER - WATCH YOUR CONSOLE
useEffect(() => {
  console.log("--- ABOUT PAGE LANGUAGE LOG ---");
  console.log("Global Active Code State:", i18n.language);
  console.log("Loaded Dictionary Keys:", i18n.store?.data?.[i18n.language]);
}, [i18n.language]);
    ...
    <div style={{ backgroundColor: '#FFF9F0', color: '#1c1917' }} className="min-h-screen flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-4xl mx-auto px-6 py-12 md:py-16">
        <h1 className={`font-black text-[#6A2301] mb-6 tracking-tight ${
          i18n.language?.startsWith('si') ? 'text-3xl md:text-4xl' : 'text-2xl md:text-3xl'
        }`}>
