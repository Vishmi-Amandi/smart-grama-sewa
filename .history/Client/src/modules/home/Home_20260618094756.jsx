...
// ─── Section anchor IDs: #about | #news | #contact ───────────────────────────
...
const Home = () => {
  ...
  // ── Icon box used in About feature cards ──────────────────────────────────
    ...
    >
      ...
      >
        ...
        {!isMobile && (
          <div className="space-x-8 font-semibold text-sm md:text-base">
            ...
            <button onClick={() => scrollTo('about')} className="bg-transparent border-none cursor-pointer font-semibold text-sm md:text-base hover:text-[#6A2301] transition-colors" style={{ color: 'inherit' }}>About</button>
            <button onClick={() => scrollTo('news')} className="bg-transparent border-none cursor-pointer font-semibold text-sm md:text-base hover:text-[#6A2301] transition-colors" style={{ color: 'inherit' }}>News &amp; Notices</button>
            <button onClick={() => scrollTo('contact')} className="bg-transparent border-none cursor-pointer font-semibold text-sm md:text-base hover:text-[#6A2301] transition-colors" style={{ color: 'inherit' }}>Contact</button>
          </div>
        )}
        ...
        <>
          ...
          <div className="fixed top-0 left-0 w-[260px] h-screen bg-[#F5C400] z-50 shadow-2xl flex flex-col py-6 px-4 animate-slide-right">
            ...
            <div className="flex flex-col gap-2 font-bold text-base text-[#3d2a00]">
              ...
              <button onClick={() => scrollTo('about')} className="text-left px-3 py-2 rounded-lg hover:bg-white/30 transition-colors bg-transparent border-none cursor-pointer font-bold text-base text-[#3d2a00]">About</button>
              <button onClick={() => scrollTo('news')} className="text-left px-3 py-2 rounded-lg hover:bg-white/30 transition-colors bg-transparent border-none cursor-pointer font-bold text-base text-[#3d2a00]">News &amp; Notices</button>
              <button onClick={() => scrollTo('contact')} className="text-left px-3 py-2 rounded-lg hover:bg-white/30 transition-colors bg-transparent border-none cursor-pointer font-bold text-base text-[#3d2a00]">Contact</button>
            </div>
          </div>
        </>
      )}
      ...
      >
        <div className="max-w-2xl text-white py-12 md:py-24">
          <h1 className={`font-black mb-4 md:mb-6 leading-tight whitespace-pre-line tracking-tight ${
            i18n.language === 'ta' ? 'text-2xl md:text-4xl'
            : i18n.language === 'si' ? 'text-3xl md:text-[44px]'
            : 'text-3xl md:text-5xl'
          }`}>
            {t('hero_title')}
          </h1>
          <p className={`mb-6 md:mb-8 font-medium text-gray-100 opacity-90 max-w-xl ${
            i18n.language === 'ta' ? 'text-xs md:text-base'
            : i18n.language === 'si' ? 'text-[13px] md:text-[17px] leading-relaxed'
            : 'text-sm md:text-lg'
          }`}>
            {t('hero_desc')}
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="border-2 border-white text-white px-7 py-3 rounded-full font-bold text-sm md:text-base hover:bg-white/10 transition-all"
            >
              News &amp; Notices
            </button>
          </div>
      ...
      <section id="about" className="py-16 md:py-24 px-6 md:px-16" style={{ backgroundColor: '#FFFBF0' }}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-[#6A2301]">Who We Are</span>
          </div>

          <h2 className="text-2xl md:text-4xl font-black text-[#3d2a00] mb-6 leading-tight">
            Bringing Government Services<br className="hidden md:block" /> Closer to Every Citizen
          </h2>

          <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-start">
            <div>
              <p className="text-sm md:text-base text-[#5a3e00] leading-relaxed mb-4">
                <strong>Smart Grama Sewa</strong> is a digital platform developed to bridge the gap between citizens and their local Grama Niladhari (GN) officers — making everyday government services accessible from anywhere, at any time.
              </p>
              <p className="text-sm md:text-base text-[#5a3e00] leading-relaxed mb-4">
                Whether you need to request a certificate, check the status of your application, or simply reach your assigned GN officer, Smart Grama Sewa puts the Grama Niladhari office at your fingertips — no queuing, no paperwork delays.
              </p>
              <p className="text-sm md:text-base text-[#5a3e00] leading-relaxed">
                Built in partnership with local government authorities, our platform is designed to serve all communities across Sri Lanka in Sinhala, Tamil, and English.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {[
                { Icon: FileText,    title: 'Certificate Requests',  desc: 'Apply for character, residence, and income certificates online.' },
                { Icon: CalendarDays,title: 'Book Appointments',     desc: 'Schedule meetings with your GN officer without visiting the office.' },
                { Icon: Bell,        title: 'Stay Informed',         desc: 'Receive announcements and notices from your local division directly.' },
                { Icon: Handshake,   title: 'Direct Communication',  desc: 'Message your assigned GN officer securely through the portal.' },
              ].map(({ Icon, title, desc }) => (
          ...
          <div className="mt-14 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { value: '14,000+', label: 'GN Divisions in Sri Lanka' },
      ...
      <section id="news" className="py-16 md:py-24 px-6 md:px-16" style={{ backgroundColor: '#fff' }}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-3">
            <span className="w-8 h-[3px] rounded-full bg-[#F5C400] inline-block" />
            <span className="text-xs font-bold uppercase tracking-widest text-[#6A2301]">Public Announcements</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-10">
            ...
            <p className="text-xs text-[#7a5c00] md:text-right max-w-xs">
              All notices below are publicly accessible — no account required.
            </p>
          </div>
          ...
          {announcementsLoading ? (
            ...
            <div className="text-center py-20 text-[#9a7c40]">
              ...
              <p className="font-semibold text-base">No public announcements at this time.</p>
              <p className="text-sm mt-1">Check back soon for updates from your local division.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          <div className="mt-12 rounded-2xl bg-[#6A2301] text-white flex flex-col md:flex-row items-center justify-between gap-5 px-8 py-7">
            <div>
              <p className="font-black text-base md:text-lg">Want notices from your GN division?</p>
              <p className="text-sm text-white/80 mt-1">Log in to receive personalised updates and track your requests.</p>
            </div>
      ...
      <section id="contact" className="py-16 md:py-24 px-6 md:px-16" style={{ backgroundColor: '#FFFBF0' }}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-3">
            <span className="w-8 h-[3px] rounded-full bg-[#F5C400] inline-block" />
            <span className="text-xs font-bold uppercase tracking-widest text-[#6A2301]">Get in Touch</span>
          </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="rounded-2xl bg-[#6A2301] text-white p-6 shadow-sm">
                ...
                <p className="text-[10px] text-white/40 mt-3 text-center">
                  National emergency services available 24/7
                </p>
              </div>
              ...
              <div className="rounded-2xl bg-white border border-[#f0e8d0] p-5 shadow-sm flex flex-wrap gap-4 items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-[#5a3e00]">
                  ...
                  <span>
                    <span className="font-bold text-[#3d2a00]">Follow for Updates: </span>
                    Sri Lanka e-Government portal &amp; divisional social pages.
                  </span>
                </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="rounded-2xl bg-white border border-[#f0e8d0] p-6 shadow-sm">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-[#3d2a00] text-sm group-hover:text-[#6A2301] transition-colors">Frequently Asked Questions</p>
                      <p className="text-[11px] text-[#7a5c00]">Find quick answers to common queries</p>
                    </div>
                  </div>
              ...
              <div className="rounded-2xl border-2 border-[#F5C400] bg-white p-6 flex flex-col md:flex-row gap-5 items-start md:items-center shadow-sm">
                ...
                <div className="flex-1">
                  <p className="font-black text-[#3d2a00] text-base mb-1">Contact Your GN Officer</p>
                  <p className="text-xs text-[#5a3e00] leading-relaxed">
                    To contact your assigned Grama Niladhari officer, please login to your citizen account. Your GN officer is linked based on your registered GN division.
                  </p>
                </div>
                <Link
                  to="/login"
      ...
      <footer style={{ backgroundColor: '#6A2301' }} className="text-white py-8 px-6 md:px-16 border-t border-black/10">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
          <div>
            ...
            <p className="text-xs text-white/60">A citizen-first digital government initiative.</p>
          </div>
          <div className="flex flex-wrap gap-4 text-xs font-semibold text-white/80">
            <button onClick={() => scrollTo('about')} className="hover:text-white transition-colors bg-transparent border-none cursor-pointer text-xs font-semibold text-white/80">About</button>
            <button onClick={() => scrollTo('news')} className="hover:text-white transition-colors bg-transparent border-none cursor-pointer text-xs font-semibold text-white/80">News &amp; Notices</button>
            <button onClick={() => scrollTo('contact')} className="hover:text-white transition-colors bg-transparent border-none cursor-pointer text-xs font-semibold text-white/80">Contact</button>
