...
import { useTranslation } from 'react-i18next'; // <-- 1. Added translation import
...
// Desktop Sidebar
const DesktopSidebar = ({ activePage, navigate, onLogout, t }) => { // <-- 2. Added t prop
  const navItems = [
    { key: 'dashboard', icon: IC.dashboard, label: t('lbl_dashboard') }, // <-- 3. Linked translation lookups
        ...
        {navItems.map((item) => (
          <NavItem key={item.key} iconPath={item.icon} label={item.label}
            ...
            onClick={() => {
              ...
              navigate(item.key === 'dashboard' ? '/dashboard' : `/${item.key}`); // <-- 4. Changed to explicit path logic
            }} 
          />
        ))}
      </div>
          ...
          <NavItem key={item.key} iconPath={item.icon} label={item.label}
            ...
            onClick={() => {
              if (item.key === 'logout') {
                ...
                navigate(item.key === 'dashboard' ? '/dashboard' : `/${item.key}`); // <-- 5. Handled bottom navigation
              }
            }} 
          />
        ))}
...
const SearchResultsDropdown = ({ searchQuery, showResults, setShowResults, setSearchQuery, navigate, t }) => { // <-- 6. Prop layers matched
  const [filteredPages, setFilteredPages] = useState([]);

  const PAGE_ACTIONS_TRANS = [
  ...
  useEffect(() => {
    ...
    const filtered = PAGE_ACTIONS_TRANS.filter(page =>
      page.name.toLowerCase().includes(query)
    );
    setFilteredPages(filtered);
  }, [searchQuery, PAGE_ACTIONS_TRANS]);
...
const PAGE_ACTIONS_STATIC_KEYS = ['dashboard', 'announcements', 'appointments', 'forms', 'ai_assistant', 'profile', 'settings'];
