...
import { useTranslation } from 'react-i18next'; // 1. Imported i18n hook
...
const StepIndicator = ({ current }) => {
  ...
  // 2. Localized step tracker lists dynamically
...
// 25 District Registry Maps mapping remains unchanged
...
// STEP 1 — About You Component Support
const Step1 = ({ data, onChange, onNext }) => {
  ...
  const handleNicChange = (e) => {
    ...
    if (nicCheckTimeout.current) clearTimeout(nicCheckTimeout.current);
    nicCheckTimeout.current = setTimeout(() => checkNicUniqueness(value), 500);
  };
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: '16px', marginBottom: '18px' }}>
        <div>
          ...
          />
          {checkingNic && <p style={{ color: '#888', fontSize: '12px', marginTop: '4px' }}>{t('nic_checking')}</p>}
          ...
          {!errors.nic && data.nic && nicAvailable && data.nic.trim().length >= 9 && !checkingNic && (
            <p style={{ color: '#30a050', fontSize: '12px', marginTop: '4px' }}>{t('nic_valid')}</p>
          )}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'flex-end', gap: '16px', marginBottom: '4px' }}>
        <div style={{ flex: 1 }} />
...
// STEP 2 — Contact Details Component Support
const Step2 = ({ data, onChange, onNext, onBack }) => {
  ...
  const handleEmailChange = (e) => {
    ...
    emailCheckTimeout.current = setTimeout(() => checkEmailUniqueness(value), 500);
  };
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '16px', marginBottom: '18px' }}>
        <div>
          ...
          <input
            ...
            style={inp(isMobile, errors.email || (!emailAvailable && !checkingEmail))}
            onFocus={(e) => (e.target.style.borderColor = '#B46A02')}
            onBlur={(e)  => (e.target.style.borderColor = errors.email ? '#e05050' : '#d4c9a8')}
          />
          {checkingEmail && <p style={{ color: '#888', fontSize: '12px', marginTop: '4px' }}>{t('email_checking')}</p>}
          ...
          {!errors.email && data.email && !checkingEmail && !emailAvailable && (
            <p style={{ color: '#e05050', fontSize: '12px', marginTop: '4px' }}>{t('err_email_taken')}</p>
          )}
          {!errors.email && data.email && emailAvailable && !checkingEmail && (
            <p style={{ color: '#30a050', fontSize: '12px', marginTop: '4px' }}>{t('email_valid')}</p>
          )}
        </div>
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: '16px', marginBottom: '28px' }}>
        <div>
          ...
          <select value={data.district} onChange={(e) => handleDistrictChange(e.target.value)} style={selectStyle(errors.district)}>
        ...
        <div>
          ...
          >
            <option value="">
              {loadingGn ? t('gn_loading') : (data.dsDiv ? t('gn_select') : t('gn_disabled'))}
            </option>
            {gnDivisions.map((gn) => (
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column-reverse' : 'row', justifyContent: 'space-between', alignItems: 'center', gap: isMobile ? '12px' : '0' }}>
...
// STEP 3 — Password & Integrity Constraints Component Support
const Step3 = ({ data, onChange, onBack }) => {
  const { t } = useTranslation();
    ...
    try {
      ...
      if (realEmail) {
        try {
          ...
          console.warn('Email verification template skip:', e.message);
        }
      }
      ...
      try {
        ...
        console.warn('updateProfile profile sync warning:', e.message); 
      }
      ...
      let friendlyError;
      if (err.code === 'auth/email-already-in-use') friendlyError = t('err_email_taken');
      else if (err.code === 'auth/invalid-email') friendlyError = t('err_email_invalid');
      else if (err.code === 'auth/weak-password') friendlyError = t('err_pw_len');
  ...
  if (verificationSent) {
    return (
      <div style={{ textAlign: 'center', padding: isMobile ? '20px' : '30px' }}>
        <div style={{ width: isMobile ? '70px' : '80px', height: isMobile ? '70px' : '80px', borderRadius: '50%', backgroundColor: '#e6f9ee', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
          <svg width={isMobile ? '32' : '38'} height={isMobile ? '32' : '38'} viewBox="0 0 24 24" fill="none" stroke="#28a745" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="20 6 9 17 4 12" />
          </svg>
        </div>
        <h2 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: 800, color: '#1a1a1a', marginBottom: '12px' }}>{t('success_title')}</h2>
        <p style={{ fontSize: isMobile ? '13px' : '14px', color: '#555', lineHeight: 1.6, marginBottom: '20px' }} dangerouslySetInnerHTML={{ __html: t('success_desc') }} />
        <p style={{ fontSize: isMobile ? '12px' : '13px', color: '#888', marginBottom: '28px' }}>{t('verification_sent_msg')}</p>
        <YellowBtn onClick={() => { window.location.href = '/login'; }} isMobile={isMobile}>{t('sign_in')}</YellowBtn>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {errors.firebase && (
        <div style={{ backgroundColor: '#fff0f0', border: '1.5px solid #f0a0a0', borderRadius: '10px', padding: '12px 16px', marginBottom: '18px', fontSize: '13px', fontWeight: 600, color: '#c00', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
          <span style={{ fontSize: '16px', flexShrink: 0 }}>⚠</span>{errors.firebase}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '16px', marginBottom: '8px' }}>
        <div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            />
            <button type="button" onClick={() => setShowPw((v) => !v)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer' }}>
              <EyeIcon open={showPw} />
            </button>
          </div>
          {data.password && (
            <div style={{ marginTop: '8px' }}>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} style={{ height: '4px', flex: 1, borderRadius: '2px', backgroundColor: i <= strength ? strengthColors[strength] : '#ddd' }} />
                ))}
              </div>
        ...
        <div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            />
            <button type="button" onClick={() => setShowConf((v) => !v)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer' }}>
              <EyeIcon open={showConf} />
            </button>
          </div>
          {data.confirm && data.confirm === data.password && <p style={{ color: '#30a050', fontSize: '12px', marginTop: '4px', fontWeight: 600 }}>{t('pw_match_valid')}</p>}
          {errors.confirm && <p style={{ color: '#e05050', fontSize: '12px', marginTop: '4px' }}>{errors.confirm}</p>}
        </div>
      </div>

      <p style={{ fontSize: '12px', color: '#666', marginBottom: '24px' }}>{t('password_match_hint')}</p>
...
// STEP 4 — Post-Validation Success Frame
const StepSuccess = ({ onDashboard }) => {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '12px 0 8px' }}>
      ...
      <h2 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: 800, color: '#1a1a1a', marginBottom: '14px' }}>{t('success_title')}</h2>
      <p style={{ fontSize: isMobile ? '13px' : '14px', color: '#444', lineHeight: 1.6, marginBottom: '28px' }} dangerouslySetInnerHTML={{ __html: t('success_desc') }} />
      <YellowBtn onClick={onDashboard} isMobile={isMobile}>{t('success_btn')}</YellowBtn>
    </div>
  );
};

// Root Parent Orchestrator Engine Layout
const SignUp = () => {
  const { t } = useTranslation();
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#fefde8', fontFamily: 'Nunito, system-ui, sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: isMobile ? '16px 16px 32px' : '16px 16px 48px' }}>
        <h1 style={{ fontSize: isMobile ? '32px' : '44px', fontWeight: 900, color: '#1a1a1a', letterSpacing: '-1px', margin: '0 0 24px', textAlign: 'center' }}>
          {t('signup')}
        </h1>
        ...
        <div style={{ width: '100%', maxWidth: '760px', backgroundColor: '#ffffff', borderRadius: '20px', padding: isMobile ? '24px 20px' : '32px 36px', boxShadow: '0 4px 24px rgba(0,0,0,0.07)' }}>
      ...
      <footer style={{ backgroundColor: '#6A2301', color: '#ffffff', textAlign: 'center', padding: isMobile ? '12px 16px' : '14px 16px', fontSize: isMobile ? '12px' : '13px', fontWeight: 600 }}>
