...
import { useTranslation } from 'react-i18next'; // 1. Import translation engine

const LanguageSwitcher = () => {
  const { i18n } = useTranslation(); // 2. Extract configuration
  ...
  // Derive current language cleanly from global state
  ...
  const lightStyles = {
    button: { color: '#1e1200' },
    dropdown: { backgroundColor: '#fff', borderColor: '#e8d5ac' },
    itemDefault: { backgroundColor: '#fff', color: '#3d2a00' },
    itemActive: { backgroundColor: '#f5f0e8', color: '#3d2a00' },
    itemHover: { backgroundColor: '#f5f0e8' },
  };
  ...
  const darkStyles = {
    ...
    itemDefault: { backgroundColor: '#1e293b', color: '#f1f5f9' },
    itemActive: { backgroundColor: '#2d3a4f', color: '#F5C400' },
    itemHover: { backgroundColor: '#334155' },
  };
            ...
            {languages.map((lang) => {
              ...
              return (
                <button
                  key={lang.code}
                  onClick={() => {
                    i18n.changeLanguage(lang.code); // 3. Change global language dynamically
                    setIsOpen(false);
                  }}
                  style={{
                    width: '100%',
