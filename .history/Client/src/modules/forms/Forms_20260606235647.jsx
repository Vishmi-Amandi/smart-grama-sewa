...
const DynamicFormModal = ({ form, onClose, inputs, setInputs, currentUser, userData, db }) => {
  ...
  const validateValue = (key, val) => {
    ...
    // 1. Strict Sri Lankan Phone Number Validation (+94 followed by exactly 9 numbers)
    if (label.includes('phone') || label.includes('mobile') || label.includes('tele') || label.includes('whatsapp') || label.includes('contact')) {
      if (strVal) {
        if (!strVal.startsWith('+94')) {
          msg = 'Phone number must explicitly start with +94 country routing prefix code.';
        } else if (!/^\+94\d{9}$/.test(strVal)) {
          msg = 'Phone prefix +94 must be followed by exactly 9 digits (e.g., +94771234567).';
        }
      }
    }

    // 2. Strict Sri Lankan NIC Mathematical Sequence Validation
    if (label.includes('nic') || label.includes('identity')) {
      if (strVal) {
        if (!/^(\d{12}|\d{9}[vVxX])$/.test(strVal)) {
          msg = 'NIC format structure must be 12 digits, or 9 digits followed by a V/X.';
        } else if (!verifySriLankanNIC(strVal)) {
          msg = 'NIC warning check: The numeric day/year sequence is invalid for Sri Lankan registrations.';
        }
      }
    }

    // 3. Age Threshold Boundaries Constraints Check
    if (label === 'age') {
      if (strVal) {
        const numericAge = Number(strVal);
        if (isNaN(numericAge) || /[^0-9]/.test(strVal)) {
          msg = 'Age input entry field cannot contain floating values or non-alphanumeric characters.';
        } else if (numericAge < 0) {
          msg = 'Age entry parameter cannot be a negative value calculation value.';
