...
// --- Multi-Step Dynamic Overlay Engine (Supports Form IDs: 1, 2, 3, 4, 5) ---
const DynamicFormModal = ({ form, onClose, inputs, setInputs, currentUser, userData, db }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (form.id === 3) maxSteps = 4;                 // Income
    if (form.id === 4) maxSteps = 3;                 // Valuation
    if (form.id === 5) maxSteps = 3;                 // Identity Card Application (Form B)
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 101, width: '100%', maxWidth: '650px', maxHeight: '90vh', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Step Indicators: Identity Card Application (Form B) */}
        {form.id === 5 && (
          <div style={{ display: 'flex', backgroundColor: '#f0e8d5', padding: '10px 20px', gap: '8px', borderBottom: '1px solid #e8d5ac', fontSize: '11px', fontWeight: 700, color: '#5a3a00', flexShrink: 0 }}>
        ...
        <form onSubmit={handleFormSubmit} style={{ padding: '24px', backgroundColor: '#fffbe8', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {form.id === 5 && (
            <>
              {/* STEP 1: DRP Header Specs & Name in Block Grid */}
              {formStep === 1 && (
                <>
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase' }}>DRP Administrative Core (පුද්ගලයන් ලියාපදිංචි කිරීමේ දෙපාර්තමේන්තුව)</span>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>01. Name Specifications (English Block Letters Only)</span>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Name to appear in the Identity Card (Cage 3 Modifications)</label>
                    <input type="text" onChange={e => handleInputChange('nicCardPreferredName', e.target.value.toUpperCase())} value={inputs.nicCardPreferredName || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Fill only if different from birth certificate tracking" />
                  </div>
                </>
              )}

              {/* STEP 2: Civil Status Metatags & Birth Registry Fields */}
              {formStep === 2 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>02. Demographic Context &amp; Birth Records</span>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Birth Certificate No. / උපන් සහතිකයේ අංකය</label>
                      <input type="text" required onChange={e => handleInputChange('nicBirthCertNo', e.target.value)} value={inputs.nicBirthCertNo || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="E.G. BC/12345" />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                      ...
                      <input type="text" required onChange={e => handleInputChange('nicPlaceOfBirth', e.target.value)} value={inputs.nicPlaceOfBirth || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Hospital / Town" />
                    </div>
                    <div>
                      ...
                      <input type="text" required onChange={e => handleInputChange('nicBirthDistrict', e.target.value)} value={inputs.nicBirthDistrict || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="E.G. Matara" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="text" required onChange={e => handleInputChange('nicOccupation', e.target.value)} value={inputs.nicOccupation || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="E.G. Software Engineer (Requires upload proof under 6 months)" />
                  </div>
                </>
              )}

              {/* STEP 3: Separated Permanent vs Postal Registry & Photo Sync */}
              {formStep === 3 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>03. Addresses &amp; Certifying Evidences</span>
                  ...
                  <div>
                    ...
                    <textarea rows={2} required onChange={e => handleInputChange('nicPermanentAddress', e.target.value)} value={inputs.nicPermanentAddress || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none', resize: 'none', fontFamily: 'inherit' }} placeholder="As registered in the GN electoral checklist" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea rows={2} onChange={e => handleInputChange('nicPostalAddress', e.target.value)} value={inputs.nicPostalAddress || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none', resize: 'none', fontFamily: 'inherit' }} placeholder="Fill only if different from permanent residence tracking" />
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Telephone Number</label>
                      <input type="text" required onChange={e => handleInputChange('nicPhone', e.target.value)} value={inputs.nicPhone || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="07XXXXXXXX" />
                    </div>
                    <div>
                  ...
                  {/* Standardized DRP Scanning Photo Upload Matrix */}
                  <div style={{ backgroundColor: '#fff', border: '1.5px dashed #B46A02', borderRadius: '12px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#1e1200' }}>Official Color Photograph Registry (35mm x 45mm)</span>
                    <input type="file" required style={{ fontSize: '12px' }} />
                    <span style={{ display: 'block', fontSize: '10px', color: '#666', lineHeight: 1.3 }}>
                      * Note: Photo must look natural without maskings or disguise configurations. Scanning data will be verified by the tracking Grama Niladhari officer.
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', backgroundColor: '#eae5d8', padding: '12px', borderRadius: '10px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: '#444', fontWeight: 600, lineHeight: 1.4 }}>
                      <strong>DRP Applicant Declaration:</strong> I declare that I am a citizen of Sri Lanka. The particulars given above are true and accurate, and I am aware that presenting false registry artifacts is a punishable offence under Act No 32 of 1968.
                    </span>
                  </div>
                </>
              )}
          ...
          <div style={{ display: 'flex', justifyBetween: 'space-between', marginTop: '16px', flexShrink: 0 }}>
            <button 
              type="button" 
