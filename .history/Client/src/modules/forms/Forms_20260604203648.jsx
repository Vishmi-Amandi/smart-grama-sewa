...
// --- View Mode Toggles (For image_323002.png Dashboard layout) ---
  const [viewMode, setViewMode] = useState('library'); // 'library' or 'myForms'
  ...
  const handleFetchMyForms = async () => {
    ...
    try {
      ...
      const list = querySnapshot.docs.map((doc, index) => {
        ...
        let prefix = "APP";
        if (data.formId === 1) prefix = "RDC";
        else if (data.formId === 2) prefix = "CC";
        else if (data.formId === 3) prefix = "IC";
        else if (data.formId === 4) prefix = "VC";
        else if (data.formId === 10) prefix = "BRC";
        
        const padIndex = String(index + 1).padStart(3, '0');
        return {
          id: doc.id,
          appNo: `${prefix}-${padIndex}`,
          ...data
        };
      });
      ...
      alert("Database error pulling history matrix: " + error.message);
    } finally {
