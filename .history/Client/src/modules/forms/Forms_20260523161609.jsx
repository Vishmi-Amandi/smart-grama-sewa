...
const Forms = () => {
  ...
  const formList = [
    ...
    { id: 7, title: "Voter Registration Form", cat: "Applications", imgSrc: "/icons/voter.png", desc: "Register or update names on the local voting list" },
...
// --- Multi-Step Dynamic Overlay Engine (Supports IDs: 1, 2, 3, 4, 5, 6, 7) ---
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 101, width: '100%', maxWidth: '650px', maxHeight: '90vh', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Step Indicators: Voter Registration Form (Form ID: 7) */}
        {form.id === 7 && (
          <div style={{ display: 'flex', backgroundColor: '#f0e8d5', padding: '10px 20px', gap: '8px', borderBottom: '1px solid #e8d5ac', fontSize: '11px', fontWeight: 700, color: '#5a3a00', flexShrink: 0 }}>
        ...
        <form onSubmit={handleFormSubmit} style={{ padding: '24px', backgroundColor: '#fffbe8', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    {hasBusinessIncome && (
                      <div style={{ marginTop: '12px', borderTop: '1px solid #f5f0e8', paddingTop: '10px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <div>
                          ...
                          <input type="text" onChange => handleInputChange('bizName', e.target.value)} value={inputs.bizName || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1.5px solid #e8d5ac', outline: 'none' }} placeholder="Business Entity Name" />
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                          <div>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
                        ...
                        <input type="text" required onChange={e => handleInputChange('boundNorth', e.target.value)} value={inputs.boundNorth || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1.5px solid #e8d5ac', outline: 'none' }} placeholder="Bounded by" />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>East (නැගෙනහිරට)</label>
                        <input type="text" required onChange={e => handleInputChange('boundEast', e.target.value)} value={inputs.boundEast || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1.5px solid #e8d5ac', outline: 'none' }} placeholder="Bounded by" />
                      </div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '16px', flexShrink: 0 }}>
            ...
            >
              {((form.id === 3 && formStep === 4) || (form.id === 6 && formStep === 4) || (form.id === 7 && formStep === 3) || (form.id !== 3 && form.id !== 6 && form.id !== 7 && formStep === 3)) ? 'Submit Application' : 'Next Step →'}
            </button>
          </div>
        </form>
      </div>
