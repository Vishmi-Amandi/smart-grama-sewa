...
const DynamicFormModal = ({ form, onClose, inputs, setInputs, currentUser, userData, db }) => {
 ...
 const validateValue = (key, val) => {
    ...
    // 1. If the field is blank, clear its error state cleanly and exit
    if (strVal === '') {
      setErrors(prev => ({ ...prev, [key]: '' }));
      return true;
    }

    // 2. Exact, safe matching for Sri Lankan Phone numbers (Checks keys specifically ending or matching phone fields)
    if (label === 'nicmobilephone' || label === 'voterchiefphone' || label === 'voterchiefwhatsapp' || label === 'treephone' || label === 'treewhatsapp') {
    ...
    // 3. Exact, safe matching for Sri Lankan NICs
    else if (label === 'nicnumber' || label === 'incnic' || label === 'voterchiefnic' || label === 'ycnicno' || label === 'treenic' || label === 'biz_owner_nic') {
    ...
    // 4. Exact check for Age boundaries
    else if (label === 'age') {
    ...
    // 5. Explicitly update the errors map object state container
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (formStep < maxSteps) {
      ...
      // 🔴 CRITICAL VOTER FORM CORRECTION: Enforce validation rules ONLY during the final form submission step!
      if (form.id === 7 && voterPurpose === 'OptionA') {
    ...
    if (formStep < maxSteps) {
      ...
      setErrors({}); // 🧹 Completely cleans up the workspace validation metrics
      onClose();
      setFormStep(1);
    }
  };
