...
// --- Multi-Step Dynamic Overlay Engine (Supports IDs: 1, 2, 3, 4, 5) ---
const DynamicFormModal = ({ form, onClose, inputs, setInputs, currentUser, userData, db }) => {
  ...
  // Custom states for Form 5 Duplicate tracking conditional inputs
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (form.id === 5) maxSteps = 4; // Updated to 4 steps to fit ALL instructions, check boxes, and receipts from the PDF
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 101, width: '100%', maxWidth: '660px', maxHeight: '90vh', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Step Indicators: Comprehensive 4-Step DRP Identity Card Application */}
        {form.id === 5 && (
          <div style={{ display: 'flex', backgroundColor: '#f0e8d5', padding: '10px 20px', gap: '8px', borderBottom: '1px solid #e8d5ac', fontSize: '11px', fontWeight: 700, color: '#5a3a00', flexShrink: 0 }}>
        ...
        <form onSubmit={handleFormSubmit} style={{ padding: '24px', backgroundColor: '#fffbe8', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {form.id === 5 && (
            <>
              {/* STEP 1: DRP Header Specs & Trilingual Form Identification Layout */}
              {formStep === 1 && (
                <>
                  ...
                  <div>
                    ...
                    <span style={{ display: 'block', fontSize: '10px', color: '#888', marginTop: '2px' }}>Leave empty if you do not carry a distinct family lineage name.</span>
                  </div>
              ...
              {/* STEP 2: Full Demographics Profile & Citizenship Status */}
              {formStep === 2 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>
                  ...
                  {/* Deep tracking for Citizens born outside Sri Lanka - Cage 7.6 / 7.8 */}
              ...
              {formStep === 3 && (
                <>
                  ...
                  {/* Conditional Duplicate Context Fields Map - Page 3 */}
                  {isDuplicateRequest && (
                    <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '4px' }}>Specific Reason Category</label>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        <div>
                          ...
                          <input type="text" required onChange={e => handleInputChange('nicLastCardNo', e.target.value)} value={inputs.nicLastCardNo || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} placeholder="Last Card Number" />
                        </div>
                        <div>
                      ...
                      {/* Lost Card Police Report tracking boxes - Cage 10.7 */}
                      <div style={{ borderTop: '1px dashed #e8d5ac', paddingTop: '8px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                          <input type="text" onChange={e => handleInputChange('nicPoliceStationName', e.target.value)} value={inputs.nicPoliceStationName || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', fontSize: '12px' }} placeholder="Name of Police Station / ස්ථානය" />
                  ...
                  {/* Trilingual Contact Enquiry Trays - Cage 11 */}
                  <div style={{ backgroundColor: '#fff', padding: '12px', borderRadius: '10px', border: '1.5px solid #e8d5ac' }}>
                    <span style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', marginBottom: '6px' }}>
                      04. Details Required For Inquiries / විමසීම් සඳහා විස්තර
                    </span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '10px', color: '#B46A02', fontWeight: 700 }}>Mobile Number / ජංගම</label>
                        <input type="text" required onChange={e => handleInputChange('nicMobileNo', e.target.value)} value={inputs.nicMobileNo || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} placeholder="07XXXXXXXX" />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '10px', color: '#B46A02', fontWeight: 700 }}>Residence Landline / ස්ථිර</label>
                        <input type="text" onChange={e => handleInputChange('nicResidencePhone', e.target.value)} value={inputs.nicResidencePhone || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} placeholder="0XXXXXXXX" />
                      </div>
                    </div>
                    <div style={{ marginTop: '8px' }}>
                      <label style={{ display: 'block', fontSize: '10px', color: '#B46A02', fontWeight: 700 }}>Email Address / විද්‍යුත් තැපෑල</label>
              ...
              {formStep === 4 && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>
                        Permanent Address / ස්ථිර පදිංචි ලිපිනය (8.1)
                      </label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>
                        Postal Address / තැපැල් ලිපිනය (8.2)
                      </label>
                  ...
                  {/* Processing Fee Receipt Ledger Fields - Page 5 */}
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>
                      Details of DRP Payments Made / ගාස්තු ගෙවීම් විස්තර (Page 5)
                    </span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>Pasted Stamp Upload / කුවිතාන්සිය</label>
                  ...
                  <div style={{ backgroundColor: '#fff', border: '1.5px dashed #B46A02', borderRadius: '12px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    ...
                    <span style={{ display: 'block', fontSize: '10px', color: '#666' }}>Must look natural without masks or artificial enhancements for scanning data integrity.</span>
                  </div>

                  {/* Fully Explanatory Comprehensive Declaration checkboxes matching all DRP points - Pages 3 & 4 */}
                  <div style={{ backgroundColor: '#eae5d8', padding: '14px', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase' }}>Declaration of the Applicant / ඉල්ලුම්කරුගේ ප්‍රකාශය</span>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', fontSize: '11px', color: '#333' }}>
                      <input type="checkbox" required defaultChecked style={{ accentColor: '#6A2301', marginTop: '2px' }} />
                      <span>
                        7. I certify that all documents furnished are legitimate and have not been subjected to forgery or alterations. I acknowledge that presenting incorrect details is a punishable offence.
                      </span>
                    </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '16px', flexShrink: 0 }}>
            ...
            >
              {((form.id === 3 && formStep === 4) || (form.id === 5 && formStep === 4) || (form.id !== 3 && form.id !== 5 && formStep === 3)) ? 'Submit Application' : 'Next Step →'}
            </button>
          </div>
        </form>
      </div>
