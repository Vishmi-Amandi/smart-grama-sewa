...
const DynamicFormModal = ({ form, onClose, inputs, setInputs, currentUser, userData, db }) => {
  ...
  // --- Living Allowance State Management Helpers ---
  const [disabledMembers, setDisabledMembers] = useState([{ name: '', relation: '', gender: '', civilStatus: '', dob: '', nic: '', nature: '' }]);
  const [otherMembers, setOtherMembers] = useState([{ name: '', relation: '', gender: '', civilStatus: '', dob: '', nic: '' }]);
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (form.id === 6) maxSteps = 3; // Living Allowance 3-Step Sequence
    ...
    if (formStep < maxSteps) {
      ...
      if (form.id === 3) {
        ...
        finalPayload = { ...inputs, disabledFamilyMembers: disabledMembers, generalFamilyMembers: otherMembers };
      }
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 101, width: '100%', maxWidth: '650px', maxHeight: '90vh', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {form.id === 6 && (
          <div style={{ display: 'flex', backgroundColor: '#f0e8d5', padding: '10px 20px', gap: '8px', borderBottom: '1px solid #e8d5ac', fontSize: '11px', fontWeight: 700, color: '#5a3a00', flexShrink: 0 }}>
            <span style={{ color: formStep === 1 ? '#6A2301' : '#888' }}>1. Administrative Region &amp; Applicant Info</span> &gt;
            <span style={{ color: formStep === 2 ? '#6A2301' : '#888' }}>2. Household &amp; Family Dynamics</span> &gt;
            <span style={{ color: formStep === 3 ? '#6A2301' : '#888' }}>3. Financial Profile &amp; Documents</span>
          </div>
        )}
        ...
        <form onSubmit={handleFormSubmit} style={{ padding: '24px', backgroundColor: '#fffbe8', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {form.id === 6 && (
            <>
              {/* STEP 1: Regional & Basic Applicant Information */}
              {formStep === 1 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>01. Regional Boundaries &amp; Demographics</span>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Divisional Secretariat</label>
                      <input type="text" required onChange={e => handleInputChange('lawDsDivision', e.target.value)} value={inputs.lawDsDivision || userData?.dsDiv || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="e.g. Thimbirigasyaya" />
                    </div>
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Grama Niladhari Division</label>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingTop: '8px', paddingBottom: '4px' }}>02. Applicant Details</span>
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Full Name of Applicant</label>
                    <input type="text" required onChange={e => handleInputChange('lawFullName', e.target.value)} value={inputs.lawFullName || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Enter Full Name" />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                      ...
                      <input type="text" required onChange={e => handleInputChange('lawDisabilityNature', e.target.value)} value={inputs.lawDisabilityNature || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Describe disability condition" />
                    </div>
                    <div>
                  ...
                  {inputs.lawDisabilityCause === 'Accident' && (
                    <div>
                      ...
                      <input type="number" required onChange={e => handleInputChange('lawAccidentYear', e.target.value)} value={inputs.lawAccidentYear || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="e.g. 2024" />
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <input type="text" required onChange={e => handleInputChange('lawEducationStatus', e.target.value)} value={inputs.lawEducationStatus || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Highest education level or trade skills" />
                  </div>
                </>
              )}

              {/* STEP 2: Household & Family Dynamics Structure */}
              {formStep === 2 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>Table A: Family Members with Disabilities</span>
                  {disabledMembers.map((member, idx) => (
                    <div key={`disabled-${idx}`} style={{ backgroundColor: '#fff', padding: '12px', borderRadius: '10px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '8px' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '10px' }}>
                        ...
                        <input type="text" placeholder="Relationship" value={member.relation} onChange={e => {
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                        ...
                        <select value={member.civilStatus} onChange={e => {
                          const updated = [...disabledMembers]; updated[idx].civilStatus = e.target.value; setDisabledMembers(updated);
                        }} style={{ padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', backgroundColor: '#fff' }}>
                          <option value="">Civil Status</option><option value="Unmarried">Unmarried</option><option value="Married">Married</option>
                        </select>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        <input type="text" placeholder="NIC Number" value={member.nic} onChange={e => {
                          const updated = [...disabledMembers]; updated[idx].nic = e.target.value; setDisabledMembers(updated);
                        }} style={{ padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} />
                        <input type="text" placeholder="Nature of Disability" value={member.nature} onChange={e => {
                          const updated = [...disabledMembers]; updated[idx].nature = e.target.value; setDisabledMembers(updated);
                        }} style={{ padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} />
                      </div>
                    </div>
                  ))}
                  <button type="button" onClick={() => setDisabledMembers([...disabledMembers, { name: '', relation: '', gender: '', civilStatus: '', dob: '', nic: '', nature: '' }])} style={{ alignSelf: 'flex-start', padding: '6px 14px', borderRadius: '6px', border: '1px solid #6A2301', color: '#6A2301', backgroundColor: '#fff', cursor: 'pointer', fontSize: '12px', fontWeight: 700 }}>+ Add Disability Member entry</button>

                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingTop: '10px', paddingBottom: '4px' }}>Table B: Other Household Members</span>
                  {otherMembers.map((member, idx) => (
                    <div key={`other-${idx}`} style={{ backgroundColor: '#fff', padding: '12px', borderRadius: '10px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '8px' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '10px' }}>
                        <input type="text" placeholder="Name" value={member.name} onChange={e => {
                  ...
                  <button type="button" onClick={() => setOtherMembers([...otherMembers, { name: '', relation: '', gender: '', civilStatus: '', dob: '', nic: '' }])} style={{ alignSelf: 'flex-start', padding: '6px 14px', borderRadius: '6px', border: '1px solid #6A2301', color: '#6A2301', backgroundColor: '#fff', cursor: 'pointer', fontSize: '12px', fontWeight: 700 }}>+ Add General Member entry</button>
                </>
              )}

              {/* STEP 3: Financial Framework & Supporting Documents */}
              {formStep === 3 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>01. Direct Bank Deposit Routing Information</span>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Bank Name &amp; Branch</label>
                      <input type="text" required onChange={e => handleInputChange('lawBankDetails', e.target.value)} value={inputs.lawBankDetails || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="e.g. Bank of Ceylon - Colombo 03" />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Account Number</label>
                      <input type="text" required onChange={e => handleInputChange('lawAccountNumber', e.target.value)} value={inputs.lawAccountNumber || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Account numeric ID string" />
                    </div>
                  </div>

                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingTop: '8px', paddingBottom: '4px' }}>02. Existing Income &amp; Aid Inventory</span>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Details of existing assistance received</label>
                      <input type="text" required onChange={e => handleInputChange('lawAidSource', e.target.value)} value={inputs.lawAidSource || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="State or NGO aid descriptions" />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Monthly Income / Assistance Value (LKR)</label>
                      <input type="number" required onChange={e => handleInputChange('lawAidAmount', e.target.value)} value={inputs.lawAidAmount || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="0.00" />
                    </div>
                  </div>

                  <div style={{ backgroundColor: '#fff', border: '1.5px dashed #B46A02', borderRadius: '12px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#1e1200' }}>Official Documentation Upload Matrix</span>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: '#444', marginBottom: '4px' }}>Certified Medical Report (Section C Upload):</label>
                      <input type="file" required style={{ fontSize: '12px', marginBottom: '6px' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: '#444', marginBottom: '4px' }}>Applicant Signature / Left Thumbprint Verification:</label>
                      <input type="file" required style={{ fontSize: '12px' }} />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', backgroundColor: '#eae5d8', padding: '12px', borderRadius: '10px', marginTop: '4px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: '#444', fontWeight: 600, lineHeight: 1.4 }}>
                      <strong>Applicant Declaration:</strong> I verify that all data vectors entered are accurate. I acknowledge that if a thumbprint is attached, it will be subjected to secondary authentication validation checks by the Grama Niladhari.
                    </span>
                  </div>
                </>
              )}
