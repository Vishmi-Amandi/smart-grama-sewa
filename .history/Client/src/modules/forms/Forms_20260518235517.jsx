...
// --- Custom Multi-Step Overlay with separated Name and Address ---
        ...
        <form onSubmit={handleFormSubmit} style={{ padding: '24px', backgroundColor: '#fffbe8', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {isResidenceOrCharacter ? (
            <>
              ...
              {formStep === 1 && (
                <>
                  ...
                  <div>
                    ...
                    <input type="text" required onChange={e => handleInputChange('applicantName', e.target.value)} value={inputs.applicantName || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="පෙළපත් නාමය සමඟ සම්පූර්ණ නම / முழுப் பெயர்" />
                  </div>

                  {/* SEPARATED ADDRESS INPUT */}
                  <div>
                    ...
                    <input type="text" required onChange={e => handleInputChange('applicantAddress', e.target.value)} value={inputs.applicantAddress || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="ස්ථිර ලිපිනය / முகவரி" />
                  </div>
              ...
              {/* STEP 2: Residence & Family context inputs */}
              {formStep === 2 && (
                <>
            ...
            <div>
              ...
              <textarea required rows={4} onChange={e => handleInputChange('generalReason', e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', resize: 'none', outline: 'none', fontFamily: 'inherit' }} placeholder={`Provide specific context matching data for ${form.title}...`} />
            </div>
          )}
