...
const [showMyFormsModal, setShowMyFormsModal] = useState(false);
...
const Forms = () => {
  ...
  // --- History Tracking Hooks for My Forms Button ---
  const [showMyFormsModal, setShowMyFormsModal] = useState(false);
  const [mySubmissions, setMySubmissions] = useState([]);
  const [loadingSubmissions, setLoadingSubmissions] = useState(false);

  const handleFetchMyForms = async () => {
    if (!currentUser) return;
    setLoadingSubmissions(true);
    setShowMyFormsModal(true);
    ...
    try {
      ...
      list.sort((a, b) => b.submittedAt?.toDate() - a.submittedAt?.toDate());
      setMySubmissions(list);
    } catch (error) {
      alert("Error pulling submitted forms: " + error.message);
    } finally {
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.main}>
        ...
        <div style={S.scrollArea}>
          ...
          <div style={{ display: 'flex', marginBottom: '20px' }}>
            ...
            {/* Connected onClick Hook Entry */}
            <button 
              onClick={handleFetchMyForms}
              style={{ marginLeft: 'auto', backgroundColor: '#6A2301', color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 32px', fontWeight: 700, cursor: 'pointer' }}
            >
              My Forms
            </button>
          </div>
      ...
      {/* 1. Modal trigger renderer hook entry placement slot */}
      <MyFormsModal isOpen={showMyFormsModal} onClose={() => setShowMyFormsModal(false)} submissions={mySubmissions} loading={loadingSubmissions} />
    </div>
  );
};

// --- 2. Tabular History View Overlay Component Layout ---
const MyFormsModal = ({ isOpen, onClose, submissions, loading }) => {
  if (!isOpen) return null;

  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 110 }} />
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 111, width: '100%', maxWidth: '700px', maxHeight: '80vh', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        
        <div style={{ backgroundColor: '#6A2301', color: '#fff', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ padding: '20px', overflowY: 'auto', flex: 1, backgroundColor: '#fffbe8' }}>
          {loading ? (
            <div style={{ textAlign: 'center', fontWeight: 'bold', color: '#6A2301', padding: '20px' }}>Connecting to Grama Sewa Cloud Database...</div>
          ) : submissions.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#888', padding: '20px', fontWeight: 600 }}>You haven't submitted any digital applications yet.</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: '#fff', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              <thead>
                <tr>
                  <th style={{ backgroundColor: '#6A2301', color: '#fff', padding: '12px', fontSize: '13px', textAlign: 'left' }}>Form Title</th>
                  <th style={{ backgroundColor: '#6A2301', color: '#fff', padding: '12px', fontSize: '13px', textAlign: 'left' }}>Date Submitted</th>
                  <th style={{ backgroundColor: '#6A2301', color: '#fff', padding: '12px', fontSize: '13px', textAlign: 'center' }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {submissions.map((sub) => (
                  <tr key={sub.id} style={{ borderBottom: '1px solid #e8d5ac' }}>
                    <td style={{ padding: '12px', fontSize: '13px', fontWeight: 700, color: '#1e1200' }}>{sub.formTitle}</td>
                    ...
                    <td style={{ padding: '12px', textAlign: 'center' }}>
                      <span style={{ 
                        ...
                        fontSize: '11px', 
                        fontWeight: 'bold',
                        backgroundColor: sub.status === 'Approved' ? '#d4edda' : sub.status === 'Rejected' ? '#f8d7da' : '#fff3cd',
