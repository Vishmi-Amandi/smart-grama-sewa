...
const DesktopSidebar = ({ activePage, navigate, onLogout, t }) => {
  const navItems = [
    { key: 'dashboard', icon: IC.dashboard, label: t('nav_dashboard', 'Dashboard'), path: '/dashboard' },
    { key: 'announcements', icon: IC.announce, label: t('nav_announcements', 'Announcements'), path: '/announcements' },
    { key: 'appointments', icon: IC.appts, label: t('nav_appointments', 'Appointments'), path: '/appointments' },
    { key: 'forms', icon: IC.forms, label: t('nav_forms', 'Forms'), path: '/forms' },
    { key: 'ai', icon: IC.ai, label: t('nav_ai', 'AI Assistant'), path: '/ai' },
  ];
  const bottomNav = [
    { key: 'profile', icon: IC.profile, label: t('nav_profile', 'Profile'), path: '/profile' },
    { key: 'settings', icon: IC.settings, label: t('nav_settings', 'Settings'), path: '/settings' },
    { key: 'logout', icon: IC.logout, label: t('nav_signout', 'Sign out'), action: 'logout' },
  ];
...
const MobileSidebar = ({ activePage, navigate, onLogout, t }) => {
  const navItems = [
    { key: 'dashboard', icon: IC.dashboard, label: t('nav_dashboard', 'Dashboard'), path: '/dashboard' },
    { key: 'announcements', icon: IC.announce, label: t('nav_announcements', 'Announcements'), path: '/announcements' },
    { key: 'appointments', icon: IC.appts, label: t('nav_appointments', 'Appointments'), path: '/appointments' },
...
const BlankPdfsSection = ({ tab }) => {
  ...
  if (filtered.length === 0) return (
    <div className="bg-user-surface border border-user-border rounded-xl p-10 text-center">
      ...
      <p className="text-user-text-lighter font-semibold mt-3 text-sm">No blank forms available in this category yet.</p>
      <p className="text-user-text-lighter text-xs mt-1">Check back later or contact your GN office directly.</p>
    </div>
  );
          ...
          <div className="flex-1 p-4 md:p-6 overflow-y-auto">
            ...
            {viewMode === 'fillable' ? (
              ...
              <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-5 flex items-start gap-3">
                ...
                <div>
                  <div className="text-sm font-bold text-red-800">Blank Official PDF Forms</div>
                  <div className="text-xs text-red-700 mt-0.5">Download the official blank form to print and fill by hand. Bring completed forms with required documents to your GN office.</div>
                </div>
              </div>
            )}
            ...
            {/* Blank PDFs View */}
            {viewMode === 'blank' && <BlankPdfsSection tab={tab} />}
          </div>
        </div>
      </div>
