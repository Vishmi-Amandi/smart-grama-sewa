...
const DynamicFormModal = ({ form, onClose, inputs, setInputs, currentUser, userData, db }) => {
  ...
  // Strict structural pattern validation checker mapping 10-digit phones and 11/12-character Sri Lankan NIC formats
  const validateField = (fieldName, value) => {
    let errMessage = '';
    const key = fieldName.toLowerCase();

    // Condition 1: Phone numbers must be only numbers of 10 digits
    if (key.includes('phone') || key.includes('mobile') || key.includes('tele')) {
    ...
    // Condition 2: NIC must be 12 numbers or legacy 9 digits followed by 2 digits and a letter V
    if (key.includes('nic') || key.includes('identity')) {
      const legacyPattern = /^\d{9}[vV]$/;
      const modernPattern = /^\d{12}$/;
      if (value && !legacyPattern.test(value) && !modernPattern.test(value)) {
        errMessage = 'NIC must be either 12 numbers or 9 numbers followed by a V.';
      }
    }

    setErrors(prev => ({ ...prev, [fieldName]: errMessage }));
    return errMessage === '';
  };
  ...
  const isCurrentStepValid = () => {
    let stepValid = true;
    // Iterate and evaluate field objects bound to the global component instance
    Object.keys(errors).forEach(k => {
      if (errors[k] && errors[k] !== '') stepValid = false;
    });
    return stepValid;
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!isCurrentStepValid()) {
      alert('Please resolve all data formatting validation constraints before continuing.');
      return;
    }
              ...
              {formStep === 2 && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', marginBottom: '4px' }}>Period of Residence in GN Division</label>
            ...
            <>
              {formStep === 1 && (
                <>
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase' }}>DRP Form "B" / Registration of Persons Act No 32 of 1968</span>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>01. Name Block Tracing (English Block Letters Only)</span>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Given Names</label>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Name to appear on the Identity Card</label>
              ...
              {formStep === 2 && (
                <>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Profession / Occupation</label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Birth Certificate Number</label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Birth District</label>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '12px', borderRadius: '10px', border: '1.5px solid #e8d5ac', marginTop: '4px' }}>
                    <span style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', marginBottom: '6px' }}>For Applicants Born Outside Sri Lanka (Section 5(2) Citizenship Act)</span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <input type="text" onChange={e => handleInputChange('nicForeignCountry', e.target.value)} value={inputs.nicForeignCountry || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', fontSize: '12px' }} placeholder="Country of Birth" />
                      <input type="text" onChange={e => handleInputChange('nicForeignCity', e.target.value)} value={inputs.nicForeignCity || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', fontSize: '12px' }} placeholder="City" />
                    </div>
                    <input type="text" onChange={e => handleInputChange('nicCitizenshipCertNo', e.target.value)} value={inputs.nicCitizenshipCertNo || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', fontSize: '12px', marginTop: '8px' }} placeholder="Citizenship Certificate Number" />
                  </div>
                </>
              )}
              ...
              {formStep === 3 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>03. Purpose of Application &amp; Duplicate Tracking (Cage 10)</span>
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '6px' }}>Purpose of application</label>
                  ...
                  {isDuplicateRequest && (
                    <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      <span style={{ fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase' }}>Duplicate Specific Entries</span>
                      <div>
                        <select required onChange={e => handleInputChange('nicDuplicateSubReason', e.target.value)} value={inputs.nicDuplicateSubReason || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', backgroundColor: '#fff' }}>
                          ...
                          <option value="Lost">10.1 Card is Lost</option>
                          <option value="Amend">10.2 To Make Changes</option>
                          <option value="Renew">10.3 To Renew Validity Period</option>
                          <option value="Damaged">10.4 Card is Damaged/Defaced/Illegible</option>
                        </select>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        <div>
                          ...
                          {errors.nicLastCardNo && <span style={S.errorMsg}>{errors.nicLastCardNo}</span>}
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '10px', fontWeight: 700, color: '#B46A02' }}>Date of Issue</label>
                          <input type="date" required onChange={e => handleInputChange('nicLastCardIssueDate', e.target.value)} value={inputs.nicLastCardIssueDate || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} />
                        </div>
                      </div>

                      <div style={{ borderTop: '1px dashed #e8d5ac', paddingTop: '8px' }}>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '4px' }}>Pertaining Police Report Details</label>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                          <input type="text" onChange={e => handleInputChange('nicPoliceStationName', e.target.value)} value={inputs.nicPoliceStationName || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', fontSize: '12px' }} placeholder="Name of Police Station" />
                          <input type="date" onChange={e => handleInputChange('nicPoliceReportDate', e.target.value)} value={inputs.nicPoliceReportDate || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none', fontSize: '12px' }} />
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '12px', borderRadius: '10px', border: '1.5px solid #e8d5ac' }}>
                    <span style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', marginBottom: '6px' }}>04. Details Required For Inquiries</span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '10px', color: '#B46A02', fontWeight: 700 }}>Residence Landline</label>
                        <input type="text" onChange={e => handleInputChange('nicResidencePhone', e.target.value)} value={inputs.nicResidencePhone || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: errors.nicResidencePhone ? '1.5px solid red' : '1.5px solid #e8d5ac', outline: 'none' }} placeholder="0XXXXXXXX" />
                        {errors.nicResidencePhone && <span style={S.errorMsg}>{errors.nicResidencePhone}</span>}
                      </div>
                    </div>
                    <div style={{ marginTop: '8px' }}>
                      <label style={{ display: 'block', fontSize: '10px', color: '#B46A02', fontWeight: 700 }}>Email Address</label>
                      <input type="email" onChange={e => handleInputChange('nicEmailAddress', e.target.value)} value={inputs.nicEmailAddress || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} placeholder="name@domain.lk" />
                    </div>
                  </div>
                </>
              )}
              ...
              {formStep === 4 && (
                <>
                  <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>05. Addresses, Financial Stamp Auditing &amp; Legal Assent</span>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Postal Address (8.2)</label>
                      <textarea rows={2} onChange={e => handleInputChange('nicAddressPostal', e.target.value)} value={inputs.nicAddressPostal || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none', resize: 'none', fontFamily: 'inherit', fontSize: '12px' }} placeholder="Fill only if separate from your permanent address registry line" />
                    </div>
                  </div>

                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Details of DRP Payments Made (Page 5)</span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>Receipt / Certificate Number</label>
                        <input type="text" required onChange={e => handleInputChange('nicPaymentReceiptNo', e.target.value)} value={inputs.nicPaymentReceiptNo || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', outline: 'none' }} placeholder="E.G. 34600051" />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>Pasted Stamp Upload</label>
                  ...
                  <div style={{ backgroundColor: '#fff', border: '1.5px dashed #B46A02', borderRadius: '12px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#1e1200' }}>Biometric Photograph Upload Box (35mm x 45mm)</span>
                    <input type="file" required style={{ fontSize: '12px' }} />
                  </div>

                  <div style={{ backgroundColor: '#eae5d8', padding: '14px', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase' }}>Declaration of the Applicant</span>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', fontSize: '11px', color: '#333' }}>
                      <input type="checkbox" required defaultChecked style={{ accentColor: '#6A2301', marginTop: '2px' }} />
                      <span>1. I declare that I am a citizen of Sri Lanka.</span>
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', fontSize: '11px', color: '#333' }}>
                      <input type="checkbox" required defaultChecked style={{ accentColor: '#6A2301', marginTop: '2px' }} />
                      <span>2. I have read and understood the trilingual instruction manual rules.</span>
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', fontSize: '11px', color: '#333' }}>
                      <input type="checkbox" required defaultChecked style={{ accentColor: '#6A2301', marginTop: '2px' }} />
                      <span>6. All particulars given above are completely true and accurate to the best of my belief.</span>
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', fontSize: '11px', color: '#333' }}>
                      <input type="checkbox" required defaultChecked style={{ accentColor: '#6A2301', marginTop: '2px' }} />
                      <span>7. I certify that all documents furnished are legitimate and have not been subjected to forgery or alterations. I acknowledge that presenting incorrect details is a punishable offence.</span>
                    </div>
                    <div style={{ borderTop: '1px solid #dcd5c8', paddingTop: '8px', marginTop: '4px' }}>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', marginBottom: '4px' }}>Applicant's Digital Signature / thumb impression</label>
                      <input type="file" required style={{ fontSize: '12px' }} />
                    </div>
                  </div>
                </>
                ...
                <>
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '10px', fontWeight: 800, color: '#B46A02' }}>D.S. Office</label>
                        <input type="text" disabled value={userData?.dsDiv || "Thimbirigasyaya"} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', backgroundColor: '#eae5d8' }} />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '10px', fontWeight: 800, color: '#B46A02' }}>G.N. Division</label>
                        <input type="text" disabled value={userData?.gnDiv || "62B / Hunupitiya"} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', backgroundColor: '#eae5d8' }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '10px' }}>
                      ...
                      <input type="text" required onChange={e => handleInputChange('dfRelationship', e.target.value)} value={inputs.dfRelationship || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', fontSize: '12px' }} placeholder="Relation" />
                      <div>
                        <input type="text" required onChange={e => handleInputChange('dfNic', e.target.value)} value={inputs.dfNic || ''} style={{ width: '100%', padding: '8px', borderRadius: '6px', border: errors.dfNic ? '1.5px solid red' : '1px solid #e8d5ac', fontSize: '12px' }} placeholder="NIC No" />
                        {errors.dfNic && <span style={S.errorMsg}>{errors.dfNic}</span>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase' }}>
                      1.2 Other Dependent Members In Family Unit
                    </span>
                ...
                <>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', marginBottom: '4px' }}>
                      Nature of Disability (Check All Applicable Cages)
                    </span>
                    
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '12px', fontWeight: 700 }}>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_blind || false} onChange={e => handleInputChange('dis_blind', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Total Blind
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_spinal || false} onChange={e => handleInputChange('dis_spinal', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Spinal Injury
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_cerebral || false} onChange={e => handleInputChange('dis_cerebral', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Cerebral Palsy
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_deaf_mute || false} onChange={e => handleInputChange('dis_deaf_mute', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Deaf &amp; Mute
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_limbs || false} onChange={e => handleInputChange('dis_limbs', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Loss of Limbs
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_autism || false} onChange={e => handleInputChange('dis_autism', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Autism
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_intellectual || false} onChange={e => handleInputChange('dis_intellectual', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Intellectual
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_multiple || false} onChange={e => handleInputChange('dis_multiple', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Multiple
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                        <input type="checkbox" checked={inputs.dis_epilepsy || false} onChange={e => handleInputChange('dis_epilepsy', e.target.checked)} style={{ accentColor: '#6A2301' }} /> Epilepsy (Severe)
                      </label>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Age of Onset Period</label>
                    ...
                    <div>
                      ...
                      <input type="text" required onChange={e => handleInputChange('dfEducationTraining', e.target.value)} value={inputs.dfEducationTraining || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Education status / training" />
                    </div>
                  </div>

                  <div style={{ backgroundColor: '#fff', padding: '12px', borderRadius: '10px', border: '1.5px solid #e8d5ac' }}>
                    <span style={{ display: 'block', fontSize: '12px', fontWeight: 800, color: '#3d2a00', marginBottom: '6px' }}>Does the disabled individual currently utilize any assistive device?</span>
              ...
              {formStep === 3 && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Current External Relief Received</label>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase' }}>
                      Bank Account Particulars For Fund Transfer (Section B.6)
                    </span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '10px', color: '#B46A02', fontWeight: 700 }}>Bank &amp; Branch Name</label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '10px', color: '#B46A02', fontWeight: 700 }}>Account Number</label>
