...
// --- Sri Lankan NIC Check Utility Matrix ---
const verifySriLankanNIC = (nic) => {
  if (!nic) return false;
  const cleanNic = String(nic).trim();
  
  // Format 1: Old Format (9 Digits followed strictly by V or X)
  if (/^\d{9}[vVxX]$/.test(cleanNic)) {
    const digits = cleanNic.substring(0, 9);
    const daysCode = parseInt(digits.substring(2, 5), 10);
    const dayValue = daysCode > 500 ? daysCode - 500 : daysCode;
    if (dayValue === 0 || dayValue > 366) return false; 
    return true;
  }
  
  // Format 2: New Format (12 Digits starting with 19 or 20)
  if (/^\d{12}$/.test(cleanNic)) {
    const yearPrefix = parseInt(cleanNic.substring(0, 4), 10);
    if (yearPrefix < 1900 || yearPrefix > new Date().getFullYear()) return false;
    
    const daysCode = parseInt(cleanNic.substring(4, 7), 10);
    const dayValue = daysCode > 500 ? daysCode - 500 : daysCode;
    if (dayValue === 0 || dayValue > 366) return false;
    return true;
  }
...
const S = {
  ...
  footer: { backgroundColor: '#6A2301', color: '#fff', textAlign: 'center', padding: '16px', fontSize: '12px', marginTop: 'auto' },
...
const DynamicFormModal = ({ form, onClose, inputs, setInputs, currentUser, userData, db }) => {
 ...
 const validateValue = (key, val) => {
    ...
    // 🔴 CRITICAL FIX: If the field is completely empty, do NOT throw an error message!
    if (strVal === '') {
      setErrors(prev => ({ ...prev, [key]: '' }));
      return true;
    }

    // 1. Phone number validation (+94 followed by 9 numbers)
    if (label.includes('phone') || label.includes('mobile') || label.includes('tele') || label.includes('whatsapp') || label.includes('contact')) {
      if (!strVal.startsWith('+94')) {
        msg = 'Phone number must start with +94 country code.';
      } else if (!/^\+94\d{9}$/.test(strVal)) {
        msg = 'Must be +94 followed by exactly 9 digits (e.g., +94771234567).';
      }
    }

    // 2. Sri Lankan NIC format and validity validation
    if (label.includes('nic') || label.includes('identity')) {
      if (!/^(\d{12}|\d{9}[vVxX])$/.test(strVal)) {
        msg = 'NIC must be 12 digits or 9 digits followed by V or X.';
      } else if (!verifySriLankanNIC(strVal)) {
        msg = 'Invalid NIC number: Birth calendar days sequence is incorrect.';
      }
    }

    // 3. Age boundary rules validation
    if (label === 'age') {
      const numAge = Number(strVal);
      if (/[^0-9]/.test(strVal)) {
        msg = 'Age must be a whole number without letters or symbols.';
      } else if (numAge < 0) {
        msg = 'Age cannot be a negative value.';
      } else if (numAge > 120) {
        msg = 'Age cannot exceed 120 years.';
      }
    }
  ...
  // Helper interceptor hook to block alphabets and symbols inside numeric element inputs
  const executeNumericKeyPressBlock = (e) => {
    // Allows action navigation control characters: Backspace, Delete, Arrow Left, Arrow Right, Tab
    if (['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight', 'Tab', 'Enter'].includes(e.key)) {
      return;
    }
    // Block key entry if it is not a direct number between 0 and 9
    if (!/^[0-9]$/.test(e.key)) {
      e.preventDefault();
    }
  };

  // Specialized interceptor hook for telephone nodes supporting only a leading plus sign followed by digits
  const executePhoneKeyPressBlock = (e) => {
    if (['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight', 'Tab', 'Enter'].includes(e.key)) {
      return;
    }
    // Allow numerical entries, and restrict plus signs to the very first index slot position
  ...
  const handleFormSubmit = async (e) => {
    ...
    // Only count an error if it contains an actual, non-empty error message string
    const activeErrors = Object.values(errors).filter(errorMsg => errorMsg && String(errorMsg).trim() !== '');
    ...
    if (formStep < maxSteps) {
      setErrors({}); // 🧠 Flushes out any temporary validation logs before advancing
      setFormStep(prev => prev + 1);
    } else {
      // 1. Gather up all the inputs fields typed by the citizen
      let finalPayload = { ...inputs };
      
      if (form.id === 3) {
        finalPayload.totalAnnualIncome = totalCalculatedIncome;
      } else if (form.id === 6) {
        finalPayload.tableAFamilyWithDisabilities = disabledMembers;
        finalPayload.tableBOtherFamilyMembers = otherMembers;
      } else if (form.id === 7) {
        finalPayload.selectedPurposeMode = voterPurpose;
        finalPayload.newlyAddedVotersTable = newVoters;
        finalPayload.deletedVotersTable = deletedVoters;
      } else if (form.id === 8) {
        finalPayload.loggedTreesArray = treeLogistics;
      } else if (form.id === 9) {
        finalPayload.timberGridDetails = timberGrid;
      }

      // 2. Package the form data nicely with administrative tracking labels
      const formSubmissionData = {
        formData: finalPayload,
        formId: form.id,
        formTitle: form.title,
        formCategory: form.cat,
        applicantUid: currentUser?.uid || "anonymous",
        applicantEmail: currentUser?.email || "",
        // Capture tracking region boundaries
        district: userData?.district || inputs.lawDistrict || inputs.voterElectoralDistrict || inputs.treeDistrict || "Colombo",
        dsDivision: userData?.dsDiv || inputs.valDsDivision || inputs.treeDsDivision || inputs.removalDsOffice || "Thimbirigasyaya",
        gnDivision: userData?.gnDiv || inputs.valGnDivision || inputs.voterGnDivision || inputs.treeGnDivision || inputs.removalGnDiv || "Hunupitiya (62B)",
        status: "Pending", // This lets the Grama Niladhari filter for new requests on their dashboard!
        createdAt: new Date().toISOString(), // Standard date-time timestamp
      };

      try {
        // 3. Send the package to Firebase Firestore asynchronously
        const collectionRef = collection(db, "submitted_forms");
        const docRef = await addDoc(collectionRef, formSubmissionData);
        
        console.log("Document securely written to the backend with ID:", docRef.id);
        alert(`Success! Your application for ${form.title} has been submitted.\nTracking Reference ID: ${docRef.id}`);
        
        // 4. Reset the view state variables and close out the modal
        setErrors({});
        onClose();
        setFormStep(1);
      } catch (dbError) {
        console.error("Firebase backend submission failed:", dbError);
        alert(`Submission Error: Could not reach the database container. Please check your internet connection.`);
      }
    }
  };
  ...
  <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Age</label>
    <input 
      type="text" 
      required 
      onKeyDown={executeNumericKeyPressBlock} // 👈 Changed to match your active helper function
      onChange={e => handleInputChange('age', e.target.value)} 
      value={inputs.age || ''} 
      style={{ width: '100%', padding: '10px', borderRadius: '8px', border: errors.age ? '1.5px solid red' : '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} 
      placeholder="e.g. 24" 
    />
  {errors.age && <span style={S.errorMsg}>{errors.age}</span>}
</div>
                    <div>
  ...
  <input 
    ...
    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: errors.nicNumber ? '1.5px solid red' : '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} 
  ...
  <input 
    type="text" 
    required 
    onChange={e => handleInputChange('incNic', e.target.value)} 
    value={inputs.incNic || ''} 
    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: errors.incNic ? '1.5px solid red' : '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} 
    placeholder="e.g. 199812345678 or 981234567V" 
  />
  ...
  {/* Corrected to use a valid Sri Lankan currency layout tag with 2 fixed decimal places */}
  <span style={{ fontSize: '24px', fontWeight: 900, color: '#F5C400' }}>
    Rs. {totalCalculatedIncome.toLocaleString('en-LK', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
  </span>
</div>
                </>
              )}
              ...
              {formStep === 2 && (
                <>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase' }}>Four Boundaries</span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>North </label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>East </label>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>South </label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>West </label>
                  ...
                  <div style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac' }}>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#6A2301', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Total Extent of Land </span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>Acres </label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>Roods </label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#B46A02', marginBottom: '2px' }}>Perches </label>
                ...
                <>
                  ...
                  <div>
                    ...
                    <select required onChange={e => handleInputChange('valLandType', e.target.value)} value={inputs.valLandType || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', backgroundColor: '#fff', outline: 'none' }}>
                      ...
                      <option value="Government (රජයේ ඉඩමකි)">Government Land </option>
                      <option value="Private (පුද්ගලික ඉඩමකි)">Private Deed Land </option>
                      <option value="Devalagam/Temple (විහාරදේවාලගම්)">Temple Property</option>
  ...
  <input 
    type="text" 
    required 
    onKeyDown={executePhoneKeyPressBlock} 
    onChange={e => handleInputChange('nicMobilePhone', e.target.value)} 
    value={inputs.nicMobilePhone || ''} 
    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: errors.nicMobilePhone ? '1.5px solid red' : '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} 
    placeholder="+947XXXXXXXX" 
  />
  {errors.nicMobilePhone && <span style={S.errorMsg}>{errors.nicMobilePhone}</span>}
</div>
                    <div>
                    ...
                    {timberGrid.map((row, idx) => (
                      <div key={`timber-row-${idx}`} style={{ backgroundColor: '#fff', padding: '14px', borderRadius: '12px', border: '1.5px solid #e8d5ac', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '10px' }}>
                          ...
                          <input type="text" required placeholder="Height (Meters)" value={row.height} onChange={e => handleTimberGridChange(idx, 'height', e.target.value)} style={{ padding: '8px', borderRadius: '6px', border: '1px solid #e8d5ac', fontSize: '12px' }} />                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
...
{/* ==========================================
        ...
        {form.id === 11 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <span style={{ fontSize: '13px', fontWeight: 800, color: '#6A2301', borderBottom: '1px dashed #e8d5ac', paddingBottom: '4px' }}>Land Identity & Ownership Ledger Registry Assessment</span>
            
            <div>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Name of the Land / Plot Registration Title</label>
              <input type="text" required onChange={e => handleInputChange('assessmentLandName', e.target.value)} value={inputs.assessmentLandName || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="e.g. Welimadawatta Lot B" />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Divisional Secretariat Division</label>
                <input type="text" required onChange={e => handleInputChange('assessmentDsDivision', e.target.value)} value={inputs.assessmentDsDivision || userData?.dsDiv || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="DS Division" />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Grama Niladhari Division Name/No.</label>
                <input type="text" required onChange={e => handleInputChange('assessmentGnDivision', e.target.value)} value={inputs.assessmentGnDivision || userData?.gnDiv || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="GN Division" />
              </div>
            </div>

            <div>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Full Name of Claimant / Registered Owner(s)</label>
              <input type="text" required onChange={e => handleInputChange('assessmentOwnerName', e.target.value)} value={inputs.assessmentOwnerName || userData?.fullName || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', outline: 'none' }} placeholder="Legal Deed Title Holder Names" />
            </div>

            <div>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 800, color: '#B46A02', textTransform: 'uppercase', marginBottom: '4px' }}>Tenure Classification Classification</label>
              <select required onChange={e => handleInputChange('assessmentTenureType', e.target.value)} value={inputs.assessmentTenureType || ''} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1.5px solid #e8d5ac', boxSizing: 'border-box', backgroundColor: '#fff', outline: 'none' }}>
                <option value="">-- Select Tenure Type --</option>
                <option value="Deeded Land">Deeded Private Land (පුද්ගලික සින්නක්කර)</option>
                <option value="LDO Lease Grant">State Lease Grant / Permit (LDO රජයේ බදු)</option>
                <option value="Co-owned Family Registry">Undivided Co-owned Plot (හවුල් අයිතිය)</option>
              </select>
            </div>

            <div style={{ backgroundColor: '#fff', border: '1.5px dashed #B46A02', borderRadius: '12px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <span style={{ fontSize: '11px', fontWeight: 800, color: '#1e1200' }}>Supporting Files (Scanned Title Deed Copy, Certified Extract or Cadastral Survey Map) *</span>
              <input type="file" required style={{ fontSize: '11px', marginTop: '4px' }} />
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', backgroundColor: '#eae5d8', padding: '12px', borderRadius: '10px' }}>
              <input type="checkbox" required defaultChecked style={{ accentColor: '#6A2301', marginTop: '2px' }} />
              <span style={{ fontSize: '11px', color: '#333', fontWeight: 600, lineHeight: 1.4 }}>
                I hereby declare that I am the lawful claimant/representative of this property boundaries asset and request ownership records check evaluation.
              </span>
            </div>
          </div>
        )}
...
>
  Back
            ...
            >
              {/* Dynamic submit condition block updated for Form 11 */}
              {((form.id === 3 && formStep === 4) || (form.id === 6 && formStep === 4) || (form.id === 7 && formStep === 4) || (form.id === 8 && formStep === 5) || (form.id === 9 && formStep === 4) || (form.id === 11 && formStep === 1) || (form.id !== 3 && form.id !== 6 && form.id !== 7 && form.id !== 8 && form.id !== 9 && form.id !== 11 && formStep === 3)) ? 'Submit' : 'Next'}
            </button>
          </div>
        </form>
      </div>
