  ...
  useEffect(()=>{
    async function load(){
      ...
      try {
        ...
        const totals = { all:appts.length, comp:appts.filter(a=>(a.status||'').toLowerCase()==='completed').length,
    ...
    async function load(){
      ...
      try {
        ...
        const gnRows = gns.map(gn=>{
          ...
          return { ...gn, lastLogin, lastAppt, lastAct, loginDays, apptDays, actDays,
  ...
  return (
    <li onClick={onClick}
      className={`flex items-center gap-3 px-4 py-2 rounded-lg cursor-pointer transition-all ${
        active?'bg-amber-700 text-white font-bold':bold?'text-amber-900 font-bold hover:bg-amber-100':'text-amber-800 hover:bg-amber-100'
      }`}
